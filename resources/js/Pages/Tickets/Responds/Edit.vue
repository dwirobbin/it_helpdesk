...
const src = ref(props.respond.data.image);

const isShow = computed(() => !!props.ticket && !!props.respond);
...
const form = useForm({
    ...
    text: props.respond.data.text,
    image: props.respond.data.image,
});
...
const onUpdate = () => form.post(route('responds.update', { respond: props.ticket.data.slug }), {
    replace: true,
...
<template>
    ...
    <Head title="Edit Tanggapan" />

    <Modal :show="isShow" @close="closeModal" maxWidth="5xl" height="116" overflow="y-auto">
        <template #header>
            <h3 class="text-xl font-semibold">Edit Tanggapan</h3>
        </template>

        <template #body>
            <form @submit.prevent="onUpdate" id="create" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-6">
                    ...
                    <div class="pt-1">
                        ...
                        <div class="flex flex-col items-center sm:flex-row sm:justify-between">
                            <div>
                                ...
                                <input id="picture-respond" type="file" accept="image/*" class="hidden" ref="input" @change="handleFileChange">
