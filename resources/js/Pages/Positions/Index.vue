...
const handleSearch = debounce(() => router.visit(route('positions.index', pickBy(queries)), {
    method: 'get',
    replace: true,
    preserveState: true,
}), 300);

const handlePerPage = () => router.visit(route('positions.index', pickBy(queries)), {
    method: 'get',
    replace: true,
    preserveState: true,
});

const handleSort = (column) => router.visit(route('positions.index', pickBy(queries)), {
    method: 'get',
    replace: true,
    preserveState: true,
    onSuccess: () => {
...
const toggleConfirmDelete = (title = null, slug = null) => {
    if (title !== null && slug !== null) {
        dataDelete.title = title;
    ...
    <AuthenticatedLayout>
        <div class="p-4 bg-white block sm:flex items-center justify-between shadow-sm border-b border-gray-200 lg:mt-1">
            <div class="w-full flex flex-col justify-left sm:flex-row sm:justify-between gap-y-3">
                <h1 class="text-xl sm:text-2xl font-semibold text-gray-900 order-2 sm:order-1">Jabatan</h1>
        ...
        <div class="p-4">
            <div class="p-4 bg-white block border border-gray-200 rounded-lg shadow-sm">
                                ...
                                <table class="table-fixed min-w-full divide-y divide-gray-200">
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        <template v-if="positions.data.length">
                                            <tr v-for="(position, index) in positions?.data" :key="index" class="hover:bg-gray-100">
                                                <td class="p-4 w-12 whitespace-nowrap text-base font-medium text-gray-900">
                                                    {{ positions.meta.from + index }}
                                                </td>
                                                <td class="p-4 whitespace-nowrap text-base font-medium text-gray-900">{{ position.title }}
                                                </td>
                                                <td v-if="hasAnyRole(['Super Admin'])" class="p-4 w-1/5 whitespace-nowrap space-x-2">
                                                    <Link :href="route('positions.edit', { position: position.slug })" method="get"
                                                        preserve-state as="button" type="button" :only="['position']"
                                                    ...
                                                    <button type="button" @click="toggleConfirmDelete(position.title, position.slug)"
                ...
                <!-- Card footer -->
                <PaginationLinks v-if="positions.data.length" :pagination="positions.meta" />
            </div>
        </div>
        ...
        <!-- Edit -->
        <Edit v-if="hasAnyRole(['Super Admin']) && position" :position="position" />

        <!-- Delete -->
        <Delete v-if="hasAnyRole(['Super Admin']) && isShowConfirmDelete" :position="dataDelete" @close-confirm-modal="toggleConfirmDelete" />
    </AuthenticatedLayout>
</template>
