...
const appLogoSrc = ref(props.setting.app_logo);

const generateUrl = (filePath) => window.location.origin + filePath;

const form = useForm({
    app_name: props.setting.app_name,
    app_footer: props.setting.app_footer,
    app_logo: props.setting.app_logo,
});

const setNullLogo = () => {
    appLogoSrc.value = null, form.app_logo = null;
}
...
const previewLogo = (file) => {
    ...
    reader.onload = (e) => (appLogoSrc.value = e.target.result);
}

watch(() => form.app_logo, (newFile) => {
    if (newFile instanceof File) {
        const isImage = newFile.type.startsWith('image/');
        if (isImage) {
            previewLogo(newFile);
        } else {
            appLogoSrc.value = null;
        }
    }
});

const onUpdate = () => form.post(route('settings.app-information.update'), {
    preserveScroll: true,
    ...
    <form @submit.prevent="onUpdate">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <TextInput id="name" v-model="form.app_name" placeholder="My App" tabindex="1" />
                <InputError class="mt-2" :message="form.errors.app_name" />
            </div>
            <div>
                <InputLabel for="footer" value="Footer Aplikasi" required />
                <TextInput type="text" id="footer" v-model="form.app_footer" placeholder="All Rights Reserved" tabindex="2" />
                <InputError class="mt-2" :message="form.errors.app_footer" />
            </div>
                ...
                <div class="flex flex-col items-center sm:flex-row sm:justify-between">
                    <div>
                        <img class="mb-4 rounded-lg w-28 h-28 sm:mb-0 object-cover"
                            :src="[appLogoSrc ?? generateUrl('/assets/images/default-img.png')]" alt="Foto Keluhan">
                    ...
                    <div class="flex items-center justify-end space-x-4 mb-1 sm:mb-0">
                        ...
                        <button type="button" v-if="appLogoSrc !== null" @click="setNullLogo"
