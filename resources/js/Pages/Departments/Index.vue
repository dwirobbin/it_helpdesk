...
const handleSearch = debounce(() => router.visit(route('departments.index', pickBy(queries)), {
    method: 'get',
    replace: true,
    preserveState: true,
}), 300);

const handlePerPage = () => router.visit(route('departments.index', pickBy(queries)), {
    method: 'get',
    replace: true,
    preserveState: true,
});

const handleSort = (column) => router.visit(route('departments.index', pickBy(queries)), {
    method: 'get',
    replace: true,
    preserveState: true,
    onSuccess: () => {
...
const toggleConfirmDelete = (section = null, slug = null) => {
    if (section !== null && slug !== null) {
        dataDelete.section = section;
    ...
    <AuthenticatedLayout>
        <div class="p-4 bg-white block sm:flex items-center justify-between shadow-sm border-b border-gray-200 lg:mt-1">
            <div class="w-full flex flex-col justify-left sm:flex-row sm:justify-between gap-y-3">
                <h1 class="text-xl sm:text-2xl font-semibold text-gray-900 order-2 sm:order-1">Department</h1>
        ...
        <div class="p-4">
            <div class="p-4 bg-white block border border-gray-200 rounded-lg shadow-sm">
                ...
                <div class="mb-1 w-full">
                    <div class="sm:flex">
                        ...
                        <div v-if="hasAnyRole(['Super Admin'])"
                            ...
                            <button type="button" @click="toggleShowCreatModal"
                                ...
                                <table class="table-fixed min-w-full divide-y divide-gray-200">
                                    <thead class="bg-gray-100">
                                        <tr>
                                            ...
                                            <th @click="handleSort('section')" scope="col"
                                                class="p-4 text-left text-xs font-medium text-gray-500 uppercase cursor-pointer">
                                                <div class="flex items-center">
                                                    Divisi
                                                    <span>
                                                        <svg v-if="queries.column === 'section' && queries.direction === 'asc'"
                                                        ...
                                                        <svg v-else-if="queries.column === 'section' && queries.direction === 'desc'"
                                                            class="w-[15px] h-[15px] ms-1.5 text-gray-500" aria-hidden="true"
                                                            xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none"
                                                            viewBox="0 0 24 24">
                                                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        <template v-if="departments.data.length">
                                            <tr v-for="(department, index) in departments?.data" :key="index" class="hover:bg-gray-100">
                                                ...
                                                <td v-if="hasAnyRole(['Super Admin'])" class="p-4 w-1/5 whitespace-nowrap space-x-2">
                                                    <Link :href="route('departments.edit', { department: department.slug })" method="get"
                                                        preserve-state as="button" type="button" :only="['department']"
                                                    ...
                                                    <button type="button" @click="toggleConfirmDelete(department.section, department.slug)"
                                        ...
                                        <template v-else>
                                            <tr>
                                                <td colspan="3" class="p-4 text-center text-gray-900">
                ...
                <!-- Card footer -->
                <PaginationLinks v-if="departments.data.length" :pagination="departments.meta" />
            </div>
        </div>
        ...
        <!-- Edit -->
        <Edit v-if="hasAnyRole(['Super Admin']) && department" :department="department" />

        <!-- Delete -->
        <Delete v-if="hasAnyRole(['Super Admin']) && isShowConfirmDelete" :department="dataDelete" @close-confirm-modal="toggleConfirmDelete" />
    </AuthenticatedLayout>
</template>
